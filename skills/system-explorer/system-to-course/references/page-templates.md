...
3. [Level Selector JavaScript](#level-selector-javascript)
4. [Shared Navigation HTML](#shared-navigation-html)
...
<body>
  ...
  <!-- ========================================
       INLINE JAVASCRIPT
       See "Level Selector JavaScript" section for the full script.
       ======================================== -->
  <script>
    /* [LEVEL_SELECTOR_JS] */
    /* [SCROLL_ANIMATION_JS] */
...
## Level Selector JavaScript

Complete JS that handles level toggling, persistence, and visited-page tracking. Paste this inline in every page's `<script>` block.

```javascript
/* ==============================================
   LEVEL SELECTOR — show/hide content by depth
   Runs BEFORE first paint to avoid flash.
   ============================================== */
(function () {
  'use strict';

  var STORAGE_KEY_LEVELS  = 'system-explorer-levels';
  var STORAGE_KEY_VISITED = 'system-explorer-visited';

  // ----- Level visibility -----

  /**
   * Read saved level state from localStorage.
   * Default: all three levels visible.
   * Returns { beginner: bool, intermediate: bool, advanced: bool }
   */
  function getSavedLevels() {
    try {
      var raw = localStorage.getItem(STORAGE_KEY_LEVELS);
  ...
  /**
   * Apply the level state to the document body.
   * Adds/removes classes: hide-beginner, hide-intermediate, hide-advanced.
   * This drives all visibility via CSS:
   *   body.hide-beginner [data-level~="beginner"]:not([data-level~="intermediate"]):not([data-level~="advanced"]) { display: none; }
   *   (and so on for each level)
   * The CSS rules are in the design system; this JS only toggles body classes.
   */
  function applyLevels(levels) {
    document.body.classList.toggle('hide-beginner',     !levels.beginner);
    document.body.classList.toggle('hide-intermediate',  !levels.intermediate);
    document.body.classList.toggle('hide-advanced',      !levels.advanced);
  }

  /**
   * Save level state to localStorage so it persists across pages.
   */
  function saveLevels(levels) {
    try {
      localStorage.setItem(STORAGE_KEY_LEVELS, JSON.stringify(levels));
    } catch (e) { /* localStorage may be full or blocked; degrade silently */ }
  }

  /**
   * Sync toggle button visual state with the levels object.
   * Each button has data-level="beginner|intermediate|advanced".
   * Active buttons get the class "level-active".
   */
  function syncButtons(levels) {
    var buttons = document.querySelectorAll('.level-toggle-btn');
  ...
  // Apply immediately (before DOM ready) to avoid flash of hidden content.
  var currentLevels = getSavedLevels();
  applyLevels(currentLevels);

  // Once DOM is ready, wire up buttons and sync state.
  document.addEventListener('DOMContentLoaded', function () {
    var buttons = document.querySelectorAll('.level-toggle-btn');
    syncButtons(currentLevels);

    for (var i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener('click', function () {
        var lvl = this.getAttribute('data-level');
        currentLevels[lvl] = !currentLevels[lvl];

        // Prevent user from hiding ALL levels — keep at least one active.
        var anyActive = currentLevels.beginner || currentLevels.intermediate || currentLevels.advanced;
        if (!anyActive) {
          currentLevels[lvl] = true; // re-enable the one they just toggled
          return;
        }

        applyLevels(currentLevels);
        syncButtons(currentLevels);
        saveLevels(currentLevels);
      });
    }
  ...
  /**
   * Record the current page in localStorage so the index can show
   * which pages the user has already read.
   */
  function trackVisit() {
    try {
      var raw = localStorage.getItem(STORAGE_KEY_VISITED);
      var visited = raw ? JSON.parse(raw) : [];
...
**CSS rules for level hiding (include in the design system block on every page):**

```css
/* Level-based visibility.
   Sections with a single level are hidden when that level is toggled off.
   Sections with multiple levels stay visible if ANY of their levels is active. */
body.hide-beginner     [data-level="beginner"]     { display: none !important; }
body.hide-intermediate [data-level="intermediate"] { display: none !important; }
body.hide-advanced     [data-level="advanced"]     { display: none !important; }

/* Multi-level sections: only hide if ALL their levels are hidden.
   For data-level="beginner intermediate", hide only if both are off. */
body.hide-beginner.hide-intermediate [data-level="beginner intermediate"] { display: none !important; }
body.hide-beginner.hide-advanced     [data-level="beginner advanced"]     { display: none !important; }
body.hide-intermediate.hide-advanced [data-level="intermediate advanced"] { display: none !important; }

/* All-level sections are never hidden (they list all three levels). */
    ...
    <ul class="nav-page-list">
      ...
      <li class="[ACTIVE_CLASS]">
        <a href="[PAGE_FILENAME].html">
          ...
          <span class="nav-page-level [LEVEL_CLASS]">[LEVEL_LABEL_SHORT]</span>
        </a>
      </li>
      <!--
        Example filled-in entries:
        <li><a href="concepts.html"><span class="nav-page-title">Concepts</span><span class="nav-page-level beginner">B</span></a></li>
        <li class="active"><a href="architecture.html"><span class="nav-page-title">Architecture</span><span class="nav-page-level intermediate">I</span></a></li>
        <li><a href="how-it-works.html"><span class="nav-page-title">How It Works</span><span class="nav-page-level intermediate">I</span></a></li>
        <li><a href="implementation.html"><span class="nav-page-title">Implementation</span><span class="nav-page-level advanced">A</span></a></li>
        <li><a href="use-cases.html"><span class="nav-page-title">Use Cases</span><span class="nav-page-level beginner">B</span></a></li>
        <li><a href="ecosystem.html"><span class="nav-page-title">Ecosystem</span><span class="nav-page-level intermediate">I</span></a></li>
        <li><a href="faq.html"><span class="nav-page-title">FAQ</span><span class="nav-page-level beginner">B</span></a></li>
        <li><a href="tradeoffs.html"><span class="nav-page-title">Trade-offs</span><span class="nav-page-level intermediate">I</span></a></li>
      -->
    </ul>

    <!-- Level selector toggles -->
    <div class="level-selector" role="group" aria-label="Content level filter">
      <button class="level-toggle-btn level-active" data-level="beginner" aria-pressed="true">
        Beginner
      </button>
      <button class="level-toggle-btn level-active" data-level="intermediate" aria-pressed="true">
        Intermediate
      </button>
      <button class="level-toggle-btn level-active" data-level="advanced" aria-pressed="true">
        Advanced
      </button>
    </div>
  </div>
...
/* Level badges in nav (compact: B / I / A) */
.nav-page-level {
  font-size: 10px;
  font-weight: 700;
  width: 16px;
...
.nav-page-level.beginner     { background: var(--color-success-light); color: var(--color-success); }
.nav-page-level.intermediate { background: var(--color-info-light);    color: var(--color-info); }
.nav-page-level.advanced     { background: var(--color-warning-light); color: var(--color-warning); }
...
.level-toggle-btn.level-active {
  background: var(--color-accent);
  border-color: var(--color-accent);
  color: #fff;
}
.level-toggle-btn.level-active:hover {
